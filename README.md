...
* OLS: OpenLAN Switch，开放局域网交换机
* OLP: OpenLAN Point，开放局域网接入点
* NAT: Network Address Translation, 网络地址转换

## 分支接入

                                       OLS(企业中心) - 10.16.1.10/24
                                                ^
                                                |
                                             Wifi(DNAT)
                                                |
...
## 区域互联
                   ...
                   192.168.1.20/24                                 192.168.1.21/24
                         |                                                 |
                        OLP -- 酒店 Wifi -->  OLS(南京) <--- 其他 Wifi --- OLP
                                                |
                                                |
                                             互联网
                                                |
                                                |
                                              OLS(上海) - 192.168.1.10/24
                                                |
                                                |
                       ------------------------------------------------------
                       ^                        ^                           ^
...
# 客户端用户拨入
客户端拨入软件(OLP)工作在用户或者企业一侧，每个Point通过拨入公网侧的虚拟交换机(OLS)可以实现多点间的跨互联网互通。 

# 服务端虚拟交换
每个拨入虚拟交换机(OLS)的客户点(OLP)就像工作在一个物理的交换机下的主机，多个虚拟交换之间通过配置Link(服务端之间主动拨入对方)可以实现跨区域互通。
